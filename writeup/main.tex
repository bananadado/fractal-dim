...
\begin{theorem}{(Perron--Frobenius).}\label{thm:pf}
    ...
    simple, satisfies $\lambda>|\lambda_i|$ for every other eigenvalue, and its
    eigenvector may be taken strictly positive.
\end{theorem}
...
Perron--Frobenius is what rules out a complex or sign-alternating dominant
eigenvalue, which would make $\|v_n\|$ oscillate rather than converge.
...
\begin{example}{(Hilbert).}\label{ex:hilbert}
    ...
    ignored by the turtle; two are needed because the four sub-cells occur in
    both handednesses. On $(A,B,F)$,
    \[
...
\begin{proposition}{(Dimension of a D0L-system).}\label{prop:d0l}
    Let $G$ be a D0L-system with substitution matrix $M$, let $\lambda$ be the
    spectral radius of $M$, and suppose the turtle interpretation admits a
    scaling factor $k>1$, meaning that $\gamma_n$ is drawn with step length
    $\ell_n=k^{-n}$ and the level-$n$ figure is a union of $\Theta(\lambda^n)$
    segments of that length. Then
    \[
...
\end{proposition}
\begin{proof}
    By Perron--Frobenius the number of drawn symbols in $\omega_n$ grows as
    $\Theta(\lambda^n)$. Covering $K$ at scale $\eps=k^{-n}$ requires
    $N(\eps)=O(\lambda^n)$ boxes, so
    \[
    \frac{\log N(\eps)}{\log(1/\eps)}\leq\frac{n\log\lambda+O(1)}{n\log k}
    \longrightarrow\frac{\log\lambda}{\log k}.
    \]
    Under the OSC the drawn segments overlap only at endpoints, so the count is
    $\Theta(\lambda^n)$ from below as well and the limit is attained.
\end{proof}
\noindent The content of \ref{prop:d0l} is a separation of concerns: $\lambda$
is \textbf{combinatorial}, obtained from the grammar alone, whereas $k$ is
\textbf{geometric}, determined by $\delta$ and the net displacement of the turtle
after one production. Neither determines the dimension by itself.

\begin{remark}{(Why the spectral radius, not a symbol count).}
    For an edge-rewriting system such as Koch ($\texttt{F}\to\texttt{F+F--F+F}$)
    one has $M=(4)$ and $\lambda=4$, recovering the naive count of \texttt{F}
    symbols. For a \emph{node}-rewriting system it fails: in
    Example~\ref{ex:hilbert} each production contains three \texttt{F}s, which
    would suggest $\log3/\log2\approx1.585$, whereas the spectral radius gives
    the correct $\log4/\log2=2$. The discrepancy is that the $A$ and $B$ symbols
    in each expansion themselves generate further \texttt{F}s at the next level.
\end{remark}
...
\hline
Cantor & \texttt{F}$\to$\texttt{FfF}, \texttt{f}$\to$\texttt{ff}
    & --- & $2/3$ & $0.630930$ \\
...
\hline
\end{tabular}
\caption{L-systems with exactly known dimension, used in \S9 as ground truth.}
\label{tab:lsystems}
\end{table}

\noindent These span $[0.63,\,2]$ and are generated by a single implementation,
so they provide a controlled test of the box-counting estimator against known
values before it is applied to cases where the answer is not known in advance.
\newpage
